...
Three more things hold the form up:
...
**A create for a record that already exists is never sent.** A create's entity id stops being a `tmp_` id only when an earlier attempt at it landed and `rewriteTempIdInPayloads` re-keyed it, and a tmp id maps in `id_remap` only once one did — so `_attempt` refuses any `create` that is past either, before `markInFlight`, and marks it dead with `kAlreadyCreatedError` instead. Three paths used to send one: a second create queued while the first was in flight (`dedupPendingMutations` replaces only `pending` rows), which the landing re-keyed to the real id and re-armed — sent, it made the record twice, and its own landing then remapped the first record's real id onto the duplicate's; a Retry of a failed create re-keyed that way; and `readyPasswordRows` reviving one. All 58 create enqueue sites use a `tmp_` id, so nothing legitimate is refused. The Outbox offers no Retry for such a row (`isCreateOfExistingRecord`); Open goes to the real record. Pinned by `sync_repository_test` › a create for a record that already exists is never sent again, and `outbox_screen_discard_test`.
