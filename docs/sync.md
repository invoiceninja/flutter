...
Companion to CLAUDE.md § Sync — non-obvious rules. The main file still states all 33 rules, one line each — it is the most-cited anchor in the repo. This doc carries the evidence behind the twenty-four that needed more than a line: the status-code map, the cursor and paging gates, the company-token guard, the offline-retry policy, and what a retry may safely repeat.
