...
class QuoteRepository extends BaseEntityRepository<Quote, QuoteApi>
  ...
  Future<bool> ensurePageLoaded({
    ...
    upsert: (byId) =>
        db.quoteDao.upsertAllPreservingDirty(companyId: companyId, byId: byId),
  );
  ...
  ///
  /// A top-up, not a bundle: quotes still load page-by-page through
  ...
  Future<void> applyRefreshDelta({
    required String companyId,
    required List<QuoteApi> bundle,
  }) => applyRefreshDeltaTemplate<QuoteApi, QuotesCompanion>(
    companyId: companyId,
    bundle: bundle,
    idOf: (a) => a.id,
    ...
    upsert: (byId) =>
        db.quoteDao.upsertAllPreservingDirty(companyId: companyId, byId: byId),
    storedUpdatedAt: db.quoteDao.updatedAtAmong,
  );
  ...
  Future<void> _writeDocuments(
    ...
    String json,
      ...
      (db.update(db.quotes)..where(
            (e) => e.companyId.equals(companyId) & e.id.equals(entityId),
          ))
          .write(QuotesCompanion(documents: Value(json)));
}
