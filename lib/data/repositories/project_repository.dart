...
class ProjectRepository extends BaseEntityRepository<Project, ProjectApi>
  ...
  /// like it did nothing): the standalone list's `client_id` chips become a
  /// SQL membership predicate; `tag_ids` is a post-decode predicate (tag
  /// ids live only in the payload JSON). Tag-filter completeness is bounded
  /// by the loaded pages; the list VM strips `tag_ids` from the server
  /// fetch and auto-chains page loads to converge.
  ...
  ///
  /// A top-up, not a bundle: projects still load page-by-page through
  ...
  Future<void> applyRefreshDelta({
  ...
  }) => applyRefreshDeltaTemplate<ProjectApi, ProjectsCompanion>(
    companyId: companyId,
    bundle: bundle,
    idOf: (a) => a.id,
    ...
    storedUpdatedAt: db.projectDao.updatedAtAmong,
  );
  ProjectsCompanion _apiToCompanion(ProjectApi a, String companyId) {
    return ProjectsCompanion.insert(
      ...
      taskRate: Value(moneyString(a.taskRate)),
  ...
  Project _fromRow(ProjectRow row) {
    ...
    return Project.fromApi(api).copyWith(
      ...
      // Same story: `current_hours` is server-computed and `toApiJson`
      // doesn't send it, so a dirty row would read 0 in the DEFAULT
      // Projects layout.
  ...
  Future<void> _writeDocuments(
    ...
    String json,
      ...
      (db.update(db.projects)..where(
            (e) => e.companyId.equals(companyId) & e.id.equals(entityId),
          ))
          .write(ProjectsCompanion(documents: Value(json)));
}
