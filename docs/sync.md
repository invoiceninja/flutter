...
**A save is reported rejected only when it was.** `awaitRow` reads a `pending` row parked in the future as a failed attempt backing off — except one the drain parked behind an unsynced `tmp_` parent whose create is still on its way, `pending` or `in_flight` (`_waitsOnLandingParent`, `OutboxDao.hasLiveCreateRowFor`), or has just landed — its `id_remap` entry, written in the transaction that re-arms the save, stands in for the row the landing then deletes. One level deep: a parent itself stuck behind a dead grandparent still reads as on its way, and the save times out into "saving in background". That is no failure: `awaitRow` keeps polling, and the row sends once the parent lands and `rewriteTempIdInPayloads` re-arms it, or times out into "saving in background". It used to say "The server rejected this save" seconds before the save went through — and that banner's Discard, which now reaches a new record's own row, would have deleted it. A parent that is `dead` or `unconfirmed` never lands by itself, so a save behind one is still reported: waiting it out claimed "saving in background" for a save that goes nowhere until the user acts, and let the edit of a record whose create was rejected reach `onSaved`. A dispatched row never carries unresolved tokens, so every real 5xx still reports. Pinned by `sync_repository_test` › a save parked behind an unsynced parent, and `entity_edit_screen_scaffold_discard_test` › saving an edit.
...
**`unconfirmed` is a state, not a retry schedule** (`OutboxState`; `outbox.state` is TEXT with no CHECK, so no migration). No drain sends it. It is left out of the "Sync first" counts with `dead` (`pendingCountForCompany`, `companiesWithActiveRows` — counting it would leave that button waiting forever) and counted with `dead` as needing the user (`attentionCountAll`, `watchAttentionCount` → the sidebar badge, and the full-sign-out review prompt). It **holds back later changes to the same record** in `hasEarlierActiveRowForEntity`, parked or not — unless it is a document upload (§ A document upload holds nothing back): Resend puts it back in line, and a full-record PUT sent ahead of it would then be overwritten by the older one. A row held behind one re-parks a minute at a time, budget untouched (`_drainOnceImpl`), because it waits on the user for as long as they take: left due, held rows came back in every 50-row `nextReady` snapshot and could starve everything queued after them. The re-park writes `kHeldBehindUnconfirmedError`, and that exact error is how Resend and Discard of the unconfirmed row find what it held and make it due again (`OutboxDao.rearmHeldBehind`) — otherwise a save the user had already made sat out the rest of its minute. It counts as a local edit still in charge (`hasActiveRows*`), heals its temp ids without being re-armed (`rewriteTempIdInPayloads`), and a banner Discard may abandon it (`findDiscardableForEntity`).
