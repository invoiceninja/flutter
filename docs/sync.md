...
**A discard abandons the *row*, not the *entity*.** `SyncRepository.discardOutboxRow` clears the record's local `is_dirty` once no `pending`/`in_flight` row is left for it — but `hasActiveRowsForEntity` cannot see a **dead** row, and a dead edit's dirty flag is what stops the next refresh (`upsertAllPreservingDirty` skips dirty ids) from clobbering the unsaved work the `SaveFailedBanner` is asking the user to retry. That was fine while the only discardable rows were the entity's own edits; it stopped being fine when `add_comment` shipped a Delete on eleven detail screens, because a note is enqueued under the **parent's** `entity_type`/`entity_id`, so discarding a queued comment released a dead edit's flag. `_reconcileDiscardedDirty` now carries the same `hasEditRowForEntity` guard as its sibling `_releaseDeadLifecycleDirty` (whose name is historical — it started as the three lifecycle verbs and now releases for every kind EXCEPT `create` / `update`, because `bulk_update` was falling through both it and `_clearReorderDirty` and freezing every row a bulk column edit touched). Unconditional, because the caller deletes the row *before* reconciling — so the probe can never match the row being discarded, and abandoning an entity's only dead edit still clears as it always did. The same row-vs-entity split decides **which row** `SaveFailedBanner`'s *Discard failed save* may abandon when the VM holds no cached id: `OutboxDao.findDiscardableForEntity` takes the newest `create`/`update` for that record in state `dead`, `unconfirmed` **or** `pending` — never `in_flight` (`discardOutboxRow` deletes such a row while leaving its request on the wire), and never another kind, since a queued `add_comment` on the same record is unrelated user work. `findDeadForEntity` is the wrong query there and was the bug: only a 422 kills a row, so a 5xx or a lost connection leaves the banner up over a `pending` row that the dead-only lookup cannot see — the tap cleared the banner and left the write to apply anyway. Both edit scaffolds share it.

A ghost create is not deleted while another attempt at it is `in_flight` — a re-save queued while the first was on the wire: the discarded row goes alone, and the in-flight one, the local record and what was made against it stay for its landing or failure to settle (`sync_repository_test` › discard). Seven more ways the tap, or the save around it, reached the wrong row. **A failed re-save drops the link to the dead row the form opened with.** Only the 422 arm did; after a 5xx the newer row sat `pending` while the VM still pointed at the old dead one, so Discard deleted that and the newer write — the one just discarded — applied anyway (`GenericEditViewModel.save`'s catch). The settings scaffold re-linked the newest dead row after *every* failed save, which undid that and swapped the fresh error for the old one; it now re-links only a server rejection with field errors, as `onSaveRejected` does. **A new record's rows are keyed on its temp id.** A create form's draft has no id, so every lookup keyed on `existingId` or the draft never ran: Discard after a 5xx left the create to go out, a 422's dead row was never linked, and the quick-create dialog's Cancel looked for dead rows only. Both scaffolds and the dialog fall back to `recoveryTempId`; the dialog reads it *before* the save, which clears it on success. A create form keeps its draft after discarding a create that never reached the server — it points at no record to leave — and the next Save mints a fresh temp id. A create that went `unconfirmed` out of sight — a background retry after the failure on screen — is shown, not dropped: the banner turns to Check / Resend / Discard and the form keeps its temp id, so a Save is refused (`UnconfirmedPriorMutationException`). The dialog's Cancel leaves such a row in the Outbox. **A discard that took the record leaves the form.** When the ghost path took a never-synced record with its create — on a form opened on it, or a create that may already have made it (`unconfirmed`), whose draft saved again could make it twice — both scaffolds leave, marked clean first (`markSaved`) so an exit guard doesn't ask to discard what is gone. The entity scaffold `go`es to the record's list (`unconfirmedRowDestination`): `/x/new` is a sibling route in the entity shell with nothing to pop — the `pop` that was here never ran — and `/x/:id/edit` would pop onto the deleted record's detail. The settings scaffold leaves as its save does (`_leave`): its design editors are pushed routes. **Discard takes the failed saves it replaced.** `SyncRepository.discardFailedSave` — both scaffolds' Discard; the Outbox still discards one row — first deletes the record's older dead saves (`OutboxDao.deleteOlderDeadSaves`), then discards the row, so `_reconcileDiscardedDirty` can release the record: after a double failure the older dead save held the dirty flag, and the record went on showing the discarded content, shielded from refresh. A never-synced record's failed create stays when a later edit is discarded — it is the record. **A landed create takes the failed saves older than it.** `recordCreateSuccess`, while the drain dispatches that create, deletes the record's older dead creates and the edits that died with them (`deleteOlderDeadSaves(includeCreates: true)`) before `rewriteTempIdInPayloads` would re-key them into failures of a record that now exists — a Retry would POST the create again, or PUT older content over the new record. A newer dead row is kept: it holds newer content than what landed. **Only a create supersedes a failed create that has not landed.** The rule lives in `SyncRepository.supersedeDeadSave`, because the form's temp id is gone by `onSaved`: a `dead` create under a `tmp_` id goes only once a newer create of the record exists. An edit of that record queues an update, which waits behind the create; deleting the create stranded the record, the update then dying as referencing a discarded one. A dead create already under a real id is stale — the record exists — and still goes. **A save that goes through takes the older failed saves too.** `supersedeDeadSave` deletes the record's dead saves older than the one it drops (`deleteOlderDeadSaves`, keeping an unlanded failed create unless a create replaced it), as Discard does. It used to take only the one the form was linked to, so after two rejections the first stayed: its old error came back on the next open, and an Outbox Retry would have PUT its content over the save that had just landed. **Both scaffolds drop the failed save a re-save replaced.** The settings scaffold never did, so its failed save re-showed a stale error on every open, and an Outbox Retry would have sent it over the newer save. **A stale banner doesn't discard what is on the wire.** The unconfirmed banner's Discard re-reads its row first, as Check and Resend do: resent from the Outbox meanwhile, a row that is only queued or has died since is still dropped, as asked, but an `in_flight` one — whose request discarding by id would leave on the wire — or a gone one only drops the banner. Pinned by `generic_edit_view_model_test` › a save that fails again, `settings_entity_edit_scaffold_save_failure_test` › Discard after a failed re-save, › a create form and › leaving the form, `entity_edit_screen_scaffold_discard_test`, `client_create_dialog_test` › failed-attempt cleanup, `client_repository_test` › a create that finally succeeds, `sync_repository_test` › discard and › supersedeDeadSave, `outbox_dao_test` › deleteOlderDeadSaves, and `save_failed_banner_resend_test` › Discard on a change resent elsewhere.

## An edit of a record whose create failed saves as a create

**An edit of a record whose create failed saves as a create — `performSave` branches on `savesAsCreate`, never `isCreate`.** A record created offline whose create the server rejected keeps its `tmp_` id and its dead `create` row, and the Outbox's Open — like any edit of it — lands on its *edit* form. That form's save used to queue an `update` (`isCreate` is `_original == null`), and the drain holds a row naming an unlanded `tmp_` id back until a create of it lands (`hasCreateRowFor` counts dead creates, so dependents survive a Retry). Only a Retry of the create could land it, and that sends the create's *old* payload: the fix made on the form could never be sent. Now both scaffolds, opening on a `tmp_` record whose newest create is `dead` (`findNewestCreateForEntity`), link that create — its own rejection, not the cascade's "References a record that could not be saved" on the edits that died with it — with `applyFailedSync(recreate: true)`, and `GenericEditViewModel.savesAsCreate` (create mode, or that link while the temp id is held) sends the save as a create under the record's temp id, `repo.create(existingTempId: recoveryTempId)`. All 26 `performSave`s that can create branch on it (`test/lint/saves_as_create_wiring_test.dart`); `isCreate` itself — titles, layout, actions, some 85 reads — is unchanged. Four pieces hold it up. **The re-create waits only for an earlier create of its record.** The per-record ordering barrier held it behind anything queued on the temp id after the first create failed: an archive, a comment, a mark-sent. Every one of those references the temp id, so it waits for a create to land. Neither ever went, the form said it was saving in the background, and the cleanup dropped the failed create as superseded. `hasEarlierActiveRowForEntity(onlyCreates:)` now counts only earlier creates for a `tmp_` create, in the drain and in its in-pass blocked set. Once the create lands, `rewriteTempIdInPayloads` re-keys and re-arms the others. **The re-create replaces the record's queued updates** (`dedupPendingMutations`): left behind it, an older update held the create back through the ordering barrier while the tmp guard held the update back until a create landed — neither ever sent. Their save queries ride on the create. **The re-create's landing drops the failed saves older than it** (§ A discard abandons the row, not the entity). **A re-save after the create already landed is refused** — `CreateAlreadyLandedException` from `dedupPendingMutations`, which rolls the local upsert back — because the record exists under a real id the form never learned: another create would make it twice, and its local copy would be a phantom. A create form keeps its temp id, so every later Save is refused, and its banner offers neither Retry nor Discard (`failedSaveAlreadyCreated`); an edit form drops back to saving an update of the real record. The drain's guard catches the same case later (§ A retry is only as safe as what the endpoint does twice). There, too, the form falls back to saving an update. It also links the refused row, so that save supersedes it. Unlinked, the refused row stayed in the Outbox and the sign-out count, and came back as a banner whenever the record was opened.

Five more things hold the form up:
- **It finds a create that fails after it opened.** A save that fails without field errors re-checks for a dead create, and the next save re-sends it. An edit dies this way when its record's create fails. The entity scaffold does this through `EntityEditScaffold.onSaveFailed`, and the settings scaffold in its own save path. Until the form was reopened, each Retry queued another edit that could never go.
- **Discarding that failed create from its edit form asks first.** The prompt sits behind Confirm actions, as the Outbox's Discard does. The discard removes the never-synced record and, on the ghost path, the unsynced records made for it, such as an invoice for a new client. It used to take one tap. `SyncRepository.discardDeletesUnsyncedRecord` decides, sharing `discardOutboxRow`'s own ghost check. It says no for a create that may already have reached the server: its record may exist there, and what was made for it is kept, so "never saved to the server" would be untrue. It also says no while another attempt at the create is on the wire, because then only the row goes.
- **The re-create carries the action the failed create was saved with.** An offline "Save & Mark Paid" rejected for a taken number is fixed on the form and re-saved with a plain Save, so the new create had no `paid`; once it landed, `recordCreateSuccess` deleted the failed create, the only row that held the action, and the invoice stayed unpaid with nothing in the Outbox. `dedupPendingMutations` now also reads the record's dead saves (`OutboxDao.deadSavesForEntity`) for a create under a `tmp_` id and carries their save query ahead of the pending rows'; `mergeSaveQuery` still lets an action on the re-save win outright.
- **The Outbox offers no Retry on a failed create a newer one replaced** (`isReplacedCreate`), and `OutboxViewModel.retry` refuses one replaced while its menu was open. Sent first, it landed its stale content, and the fix was then refused as a create of an existing record.
- **A field the server freezes on update stays locked in a re-create** (known limitation). This covers the party on a billing document, payment or project: layout and validations keep `isCreate`. So a create rejected for its client can't be fixed in place; discard it and create the record again.

Pinned by `entity_edit_screen_scaffold_recreate_test`, `settings_entity_edit_scaffold_save_failure_test` › an edit of a record whose create failed, `generic_edit_view_model_test` › savesAsCreate and › a create that already landed, `client_repository_test` › re-sending the create of a record whose create failed, `save_query_dedup_test` › re-sending a failed create, `save_failed_banner_test` › a create the server already made, `sync_repository_test` › a re-sent create is not held behind the changes waiting for it, and `entity_edit_screen_scaffold_discard_test` › with Confirm actions on and › a create that fails after the form opened.
...
**A save is reported rejected only when it was.** `awaitRow` reads a `pending` row parked in the future as a failed attempt backing off — except one the drain parked behind an unsynced `tmp_` parent that lands without the user (`_waitsOnLandingParent` → `_parentLands`). That covers three cases:
- **Its create is `in_flight`.** `OutboxDao.liveCreateRowFor` prefers the attempt on the wire to a newer one queued behind it, which the drain refuses once the first lands.
- **It is queued** with no `unconfirmed` row of its record ahead of it, and every record *it* refers to lands too, checked four levels deep with a visited set.
- **It has just landed.** Its `id_remap` entry, written in the transaction that re-arms the save, stands in for the row the landing then deletes.

What the bounded walk can't tell reads as landing, the answer from before it looked past the first level. That is no failure: `awaitRow` keeps polling, and the row sends once the parent lands and `rewriteTempIdInPayloads` re-arms it, or times out into "saving in background". It used to say "The server rejected this save" seconds before the save went through — and that banner's Discard, which now reaches a new record's own row, would have deleted it. A parent that is `dead` or `unconfirmed` never lands by itself, so a save behind one is still reported: waiting it out claimed "saving in background" for a save that goes nowhere until the user acts, and let the edit of a record whose create was rejected reach `onSaved`. A dispatched row never carries unresolved tokens, so every real 5xx still reports. A dependent stripped of a tag whose create died (`_stripFailedTagFromDependents`) waits on nothing any more, so it is made due at once, as a resolved reference makes it (`OutboxDao.replacePayloadAndRearm`): parked, it sat out its minute while its form reported a rejection. One residual: `awaitRow` reads a row that has vanished as a success, so a dependent deleted because the user discarded its parent elsewhere during the ≤30 s wait reports "Saved". No data is at risk. Pinned by `sync_repository_test` › a save parked behind an unsynced parent and › tag-create 422 salvages dependents, `outbox_dao_test` › liveCreateRowFor, and `entity_edit_screen_scaffold_discard_test` › saving an edit.
...
**A create for a record that already exists is never sent.** A create's entity id stops being a `tmp_` id only when an earlier attempt at it landed and `rewriteTempIdInPayloads` re-keyed it, and a tmp id maps in `id_remap` only once one did — so `_attempt` refuses any `create` that is past either, before `markInFlight`, and marks it dead with `kAlreadyCreatedError` instead. `markInFlight` itself claims the row only while it is still `pending` and returns whether it did: a Discard or a superseding save can land between the drain's re-read and the claim (the `id_remap` lookup above awaits inside that window), and a row no longer pending is not sent (`outbox_dao_test` › markInFlight). Three paths used to send one: a second create queued while the first was in flight (`dedupPendingMutations` replaces only `pending` rows), which the landing re-keyed to the real id and re-armed — sent, it made the record twice, and its own landing then remapped the first record's real id onto the duplicate's; a Retry of a failed create re-keyed that way; and `readyPasswordRows` reviving one. All 58 create enqueue sites use a `tmp_` id, so nothing legitimate is refused. The Outbox offers no Retry for such a row (`isCreateOfExistingRecord`); Open goes to the real record. Pinned by `sync_repository_test` › a create for a record that already exists is never sent again, and `outbox_screen_discard_test`.
...
**A failed change is still unsynced work.** A `dead` row is a change the server rejected, parked together with its dirty local row so the edit form reopens onto it with a `SaveFailedBanner` (fix, then Retry). The logout / company-switch prompt counts only non-`dead` rows — right for its "Sync first" button, which cannot send a row the server refused — but every *involuntary* session end read that same count, so an idle timeout, a client-too-old sign-out, `restore()` finding no company rows, or a failed Danger Zone switch wiped rejected edits with no prompt at all. Now: `SyncRepository.hasUnsyncedWork()` reads `OutboxDao.companiesWithUnsyncedRows()` (any state), so idle timeout and client-too-old preserve the store for a dead row too; `AuthRepository.restore()` with missing account/company rows ends the session but keeps the store when the outbox holds anything (no re-lock gate — nothing was unlocked); Danger Zone's fallback logout keeps it when another company has unsynced work, and `restore()` now ignores a persisted company id that no longer names a company it holds (that fallback can leave one pointing at the deleted company). A full sign-out (`confirmPendingOutboxIfAny(checkAllCompanies: true)`) asks separately, after the pending prompt, about every row that needs the user — `dead`, and `unconfirmed` (§ A change that may already have gone through waits for the user) — with Cancel focused and View opening the Outbox of a company that has such a row (`outboxLocationNeedingAttention` — the active one when it does); a company switch never does, because the database survives it. Nothing the pending prompt's Discard chose goes until that review is passed too: it went at once, so a Cancel in the review cancelled the sign-out with the pending changes already gone. Nor does a drain send those changes meanwhile. `confirmPendingOutboxIfAny` holds drains while the review is open (`SyncRepository.holdDrains`). Without the hold, a reconnect's drain sent what the user had just chosen to discard.
- **The hold waits for nothing.** A running pass stops at its next row. `cancel()`, which logout uses, waits for a request already on the wire, and the review sat behind it with no spinner.
- **Holds nest.** One flow's release can't lift another's; `cancel()`'s latch is a single flag.
- **It is always released**, whatever the answer, because a caller may proceed and then fail to sign out.
- **A review that doesn't proceed kicks a drain,** so the changes it kept go now rather than at the next trigger. The Outbox takes `?company=` for another company's queue, which it names, offering only what stays on the device — Discard, Copy, the inspector — because Check, Resend, Retry and Open act on the active company (`stripTransientQuery` keeps the parameter out of persisted routes). Entering the Outbox tab always starts from its first page (`entersAtInitialLocation`), and a company switch drops the parameter (`companySafeLocation`). Before, the tab restored another company's queue beside a badge counting this one's. Discarding a row there fails its dependents without announcing them here (`_markDead(announce:)`): they are that company's changes, and this shell's View opens this company's Outbox. Pinned by `sync_repository_test` › hasUnsyncedWork, `auth_repository_test` › "a stale token with queued outbox work…", and `confirm_pending_outbox_test`.
