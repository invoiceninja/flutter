...
class PaymentRepository extends BaseEntityRepository<Payment, PaymentApi>
  ...
  ///
  /// A top-up, not a bundle: payments still load page-by-page through
  ...
  Future<void> applyRefreshDelta({
  ...
  }) => applyRefreshDeltaTemplate<PaymentApi, PaymentsCompanion>(
    companyId: companyId,
    bundle: bundle,
    idOf: (a) => a.id,
    ...
    storedUpdatedAt: db.paymentDao.updatedAtAmong,
  );
  ...
  Future<void> _writeDocuments(
    ...
    String json,
      ...
      (db.update(db.payments)..where(
            (e) => e.companyId.equals(companyId) & e.id.equals(entityId),
          ))
          .write(PaymentsCompanion(documents: Value(json)));
}
