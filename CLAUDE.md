...
- **A reset quarantines the store and an open imports its non-cache tables, once — the `.salvage` marker is taken *before* the import, because a second import resends delivered outbox rows — reading the snapshot with the key the open used, never a second keychain read.** → `docs/migrations.md` § A reset carries the user's own tables across
- **A reset's outcome reaches the user once per launch, never over the lock screen — a toast when everything came across, a dialog when unsynced work may not have — and a copy it keeps leaves the device only through Device Settings → Data.** → `docs/migrations.md` § What the user is told about a reset
- **One process per store: `holdStoreLock` runs before the key is read and before any reset moves the store, and refuses only a lock another process holds — `inUse` gets no Reset.** → `docs/migrations.md` § A second copy of the app never opens the store
...
- **Store SDKs (Google Sign-In, in-app purchase, Sentry) are imported only from `packages/store_services`, never from `lib/` or the root pubspec — the F-Droid build swaps that package for its FOSS twin.** → `docs/fdroid.md` § Store SDKs are imported only from packages/store_services
