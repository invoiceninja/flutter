...
- [`dashboard-panels.md`](dashboard-panels.md) — **Dashboard panels.** The main file carries the registry shape — how a panel is declared, ordered and hidden — and the rule for each behaviour; this doc carries the evidence, almost all of it from the task-calendar panel, the first Drift-backed one.
...
- [`rich-text-editing.md`](rich-text-editing.md) — **Rich text editing.** `MarkdownTextField` is the shared WYSIWYG editor for every markdown-bearing field.
...
- [`sync.md`](sync.md) — **Sync — the non-obvious rules, with evidence.** The main file still states all 27 rules, one line each — it is the most-cited anchor in the repo.
