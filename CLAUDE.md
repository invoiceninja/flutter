...
| Sync / outbox / 400-401-403-404-409-412-422 behavior | § Sync — non-obvious rules (all 29 rules) · `docs/sync.md` (the evidence) |
...
- **A failed database open destroys the store only when a fresh store fixes it** (corrupt, failed upgrade, schema drift); a lock — on web, just a second tab — a full disk or an unrecognised error leaves it untouched. → `docs/migrations.md` § A failed open destroys the store only when a fresh store fixes it
