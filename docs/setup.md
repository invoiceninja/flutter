...
**Minimum OS versions.** iOS **14.0** — `ios/Podfile` plus the three project-level `IPHONEOS_DEPLOYMENT_TARGET` entries in `ios/Runner.xcodeproj/project.pbxproj` (the Runner and RunnerTests targets declare none and inherit; `ios/Flutter/AppFrameworkInfo.plist` deliberately carries no `MinimumOSVersion` key — modern Flutter strips it). That's one step above Flutter's own 13.0 default, raised for `file_picker` ≥ 12, the only plugin in the graph that needs it — which is why a hand-built Xcode archive takes a prep step (`tools/prepare_ios_archive.sh`, § Release builds with Sentry). macOS **10.15** — `macos/Podfile` + `MACOSX_DEPLOYMENT_TARGET`, matching Flutter's default. Android pins nothing of its own; `android/app/build.gradle.kts` tracks `flutter.minSdkVersion`.

Each platform ships via its own release workflow — see § Shipping to the stores.

### App icons

`dart run tools/gen_app_icons.dart` regenerates **every derived icon raster** from the single 1024px macOS source (`macos/AppIcon.icon/Assets/Image.png`) — run it after any logo change and commit the outputs:

| Output | Shape | Used by |
|---|---|---|
| `windows/runner/resources/app_icon.ico` (16–256) | disc | the `.exe`, title bar, Alt-Tab, Explorer |
| `windows/runner/resources/store_logo.png` (1024) | disc | `msix_config.logo_path` → **every** MSIX tile (Start, taskbar, Search, splash, Store) |
| `web/icons/Icon-{192,512}.png` | disc | PWA install, `apple-touch-icon` |
| `web/icons/Icon-maskable-{192,512}.png` | opaque square | `purpose: maskable` in `web/manifest.json`, plus the `og:image` / `twitter:image` cards |
| `snap/gui/invoiceninja.png` (512) | disc | Snap and AppImage (`tools/build_appimage.sh`) |

iOS/macOS consume the source directly (`AppIcon.icon`) and Android is generated by `flutter_launcher_icons` from `assets/images/icon.png`, so neither is in scope.

The source is a macOS "glass" **alpha matte** — its RGB is entirely black, so it must be baked (`255 - alpha`) before any other platform can use it; feeding it through raw yields a black silhouette. The disc shape (mark on a black circle, transparent outside) matches what v1 has always shipped on Windows and is what a Windows 11 taskbar needs, since MSIX draws the app icon *unplated*. `test/lint/app_icons_test.dart` guards all of this — see [#25](https://github.com/invoiceninja/flutter/issues/25), where a package built without `logo_path` shipped msix's own Flutter-logo tiles.

The user-facing Windows name lives in `windows/runner/main.cpp` (window title), `windows/CMakeLists.txt` (`BINARY_NAME`), and `windows/runner/Runner.rc` (version-info strings).

## Shipping to the stores

> **Setting these up from scratch?** `docs/store-deployment-setup.md` is the step-by-step runbook — every account, role, and secret in one ordered checklist. This section is the reference: what each pipeline does and why it's built that way.
...
**Every one of them gates on the test suite first.** Each workflow has two jobs: a `test` job that calls the reusable `.github/workflows/_test.yaml` (`flutter pub get` → `build_runner` → `flutter test` over the whole `test/` tree, always on `ubuntu-latest`), and the `deploy-*` job with `needs: test`. A red suite therefore stops a release before anything is built or uploaded. This matters because `ci.yaml` is `workflow_dispatch` only — without the gate, nothing in the pipeline would have run the tests for the commit being shipped. It adds roughly 8-12 min to a dispatch. `_test.yaml` is the single definition of "run the tests" (`ci.yaml`'s `unit-tests` job calls it too) — don't inline a second copy of `flutter test` into a release workflow.
...
**Per release.** Run `tools/bump_client_version.sh`, commit, then dispatch. The build number must **strictly increase** for Play / iOS / macOS / Windows (each rejects a re-used version). **Snap is the exception** — it's revision-ordered, so no bump is needed to publish; its display version in `snap/snapcraft.yaml` (`version:`) is rewritten by the same bump script, and `test/app/version_test.dart` asserts it matches the pubspec triplet. **AppImage is the other exception** — its tag/asset come from the pubspec display version (`v<version>`), so re-dispatching the same version updates that GitHub Release in place; cut a *new* release by bumping the version (the bump script advances the patch, e.g. `5.1.4` → `5.1.5`, yielding a fresh `v5.1.5` tag).
...
- **Build mechanism.** `tools/build_appimage.sh` (also the local builder — Linux host only) compiles `flutter build linux --release` with the DSN baked in, then packages via `linuxdeploy --plugin gtk` (bundles GTK3 + gdk-pixbuf loaders + GSettings schemas). The whole Flutter bundle goes into `AppDir/usr/bin/` so `admin`, `lib/`, and `data/` stay together — the embedder resolves `lib/libapp.so` + `data/` via `/proc/self/exe`. The tools (linuxdeploy, the gtk + appimage plugins) and the AppImage runtime are downloaded at build time; checksum pinning is wired (`appimage/tool-checksums.sha256`) but that file currently holds **no entries**, so the script prints `WARNING: … skipping verification` and runs unpinned — the build logs the observed hashes, so pin it by copying them in. CI sets `APPIMAGE_EXTRACT_AND_RUN=1` (the runners have no libfuse2). No `execstack` step (unconfined).
...
- **Repo secrets** (Settings → Secrets and variables → Actions):
  ...
  - `IN_GOOGLE_SERVER_CLIENT_ID` (optional) — switches "Sign in with Google" on; § Google Sign-In client IDs.
...
- **Repo secrets** (Settings → Secrets and variables → Actions):
  ...
  - Reuses the existing `IN_SENTRY_DSN` secret for symbolicated crash reports.
  - `IN_GOOGLE_IOS_CLIENT_ID` (optional, iOS job only) — switches "Sign in with Google" on; § Google Sign-In client IDs.
...
The unsigned `.msix` is submitted to the existing `InvoiceNinja.AdminPortal` Microsoft Store listing by the manually-triggered `.github/workflows/microsoft-store.yml` (`workflow_dispatch` only — publishing is a deliberate release action, not a per-push gate). It mirrors `playstore.yml`: a `test` gate plus one deploy job on `windows-latest`, store credentials as repo secrets, `IN_SENTRY_DSN` baked in at compile time. (The gate runs on `ubuntu-latest`, not Windows — the suite has never been exercised on Windows, so a stray path-separator assumption shouldn't be able to red a release.) The Microsoft Store has **no instant testers-only channel** like the snap's `edge`, Play's `internal`, or TestFlight, so the workflow **auto-submits (commits)** the submission: a dispatch uploads the package and commits it, so it enters Store **certification** automatically and goes live on the (restricted/beta) listing once certified — there is no manual gate. To add a safety gate later, append `--noCommit` to the publish step (keeps the submission a draft you click Publish on in Partner Center) or target a closed-beta **package flight** with `-f <flightId>`.
...
- **MSIX version must always increase.** The package version auto-derives from the pubspec `version:` (`5.1.6+9` → `5.1.6.0`, format `x.y.z.0`), and each Store submission must use a strictly higher version. It's advanced only by `tools/bump_client_version.sh`. **Run the bump, commit it, then trigger the workflow** — re-dispatching without a bump produces a duplicate version the Store rejects. Certification then runs (typically hours) before the build is live; there is no instant promotion.
- **Repo secrets** (Settings → Secrets and variables → Actions):
...
Routine bump: raise the `^` floors in `pubspec.yaml`, run `flutter pub upgrade`, regenerate codegen (`dart run build_runner build --delete-conflicting-outputs`), and — only if `drift` or `sqlite3` moved — the vendored web assets (§ Web setup notes). Gate the result with `flutter analyze` + `flutter test` + `flutter build web --wasm`. If `sqlite3` moved, also re-pin the `SQLITE3MC_*` URL and hashes in `tools/prepare_foss.sh` to the sqlite3mc release the new version ships prebuilt (`docs/fdroid.md` § Bumping sqlite3mc). The store SDKs (`google_sign_in`, `in_app_purchase*`, `sentry_flutter`) are bumped in `packages/store_services/pubspec.yaml`, not here.

**Why `flutter pub outdated` still shows a stale tail.** ~21 transitive packages list a newer **"Latest"** that is *not resolvable*: `flutter pub upgrade --major-versions --dry-run` (the most aggressive solve — it even rewrites our own constraints) reports **"No dependencies would change."** Each is capped by an **already-latest upstream package** (not by our `pubspec.yaml`), so nothing here can move them; they clear only when the upstream widens its bound or Flutter ships a newer stable. Verified 2026-06-02 on Flutter 3.44.1 / Dart 3.12.1:
...
**iOS archives need one terminal command first (SwiftPM).** Before every hand-built iOS **Product → Archive**, run **`tools/prepare_ios_archive.sh`** from a terminal. It runs `flutter build ios --release --config-only`, then verifies what that produced — checking the regenerated manifest against the project's `IPHONEOS_DEPLOYMENT_TARGET` *and* against the highest floor any linked plugin declares, so a future plugin bump that outgrows the project target fails loudly and by name instead of leaving you to decode the Xcode error again. Without it Xcode fails package resolution with *"The package product 'file-picker' requires minimum platform version 14.0 for the iOS platform, but this target supports 13.0"* — even though `ios/Podfile` and all three `IPHONEOS_DEPLOYMENT_TARGET` entries in `ios/Runner.xcodeproj/project.pbxproj` already say 14.0. The 13.0 lives in the *generated, gitignored* `ios/Flutter/ephemeral/Packages/FlutterGeneratedPluginSwiftPackage/Package.swift`, which is the top-level SwiftPM package the Runner target links: `flutter_tools` writes it at its hardcoded iOS default (`darwin.dart` → 13.0) and only raises it to the project's real target in `SwiftPackageManager.updateMinimumDeployment`, called from **one** place — inside `flutter build ios`/`ipa`/`run` (`ios/mac.dart`). An Xcode archive never calls it, and **every** `flutter pub get` (including an IDE auto-pub-get on saving `pubspec.yaml`) resets the file to 13.0 — so this is a per-archive step, not a one-time fix. **No scheme pre-action can do it for you:** Xcode resolves the Swift package graph *before* any pre-action or build-phase script runs, so a hook would at best fix the *next* archive — and running `--config-only` from a pre-action is exactly what deadlocked iOS archives above. Upstream: [flutter/flutter#162196](https://github.com/flutter/flutter/issues/162196) (open, P3) — `--config-only` is the Flutter team's own stated workaround; tracked in [`upstream-workarounds.md`](upstream-workarounds.md) § 8. The 14.0 floor comes from `file_picker` ≥ 12, the only plugin in the graph above 13.0. macOS is unaffected — Flutter's macOS default (10.15) already equals `MACOSX_DEPLOYMENT_TARGET`. CLI and CI are unaffected too: `tools/build_release.sh ios` goes through `flutter build ipa`, and `appstore-ios.yml` already runs the same `--config-only` step before its `xcodebuild archive`. Running it first also *helps* the DSN pre-action above — it guarantees the `DART_DEFINES` line that script patches in place already exists.
...
"Sign in with Google" (login, sign-up, and Settings → User Details → Connect) is **off in any build that lacks the client ID for its platform** — `GoogleSignInClient.isSupported` hides the buttons rather than showing ones that fail. The IDs are compile-time dart-defines, never committed, so a fork or the F-Droid build never ships Invoice Ninja's OAuth project:

| Platform | Dart-define | Where it's used |
|---|---|---|
| Android | `IN_GOOGLE_SERVER_CLIENT_ID` | `initialize(serverClientId:)` — the **Web** client ID (Credential Manager can't read it from `google-services.json`) |
| iOS | `IN_GOOGLE_IOS_CLIENT_ID` | `initialize(clientId:)` — the **iOS** client ID; **plus** its reversed form as a URL scheme in `ios/Runner/Info.plist` |

Web, macOS, Windows and Linux have no Google sign-in (admin-portal had none there either; web is email/password only by decision).

**Why new IDs are needed.** An OAuth client is bound to one bundle / package id. admin-portal's clients (Cloud project `640903115046`) are for `com.invoiceninja.app`; this app is `com.invoiceninja.admin`, so reusing them fails with `DEVELOPER_ERROR` (Android) or a bundle mismatch (iOS).

**One-time Google Cloud setup** (Cloud Console → APIs & Services → Credentials, in project `640903115046` so the existing consent screen and Web client are shared):

1. **iOS client** — Create credentials → OAuth client ID → *iOS*, bundle id `com.invoiceninja.admin`. Note the client ID (`<n>-<hash>.apps.googleusercontent.com`) and the *iOS URL scheme* it shows (`com.googleusercontent.apps.<n>-<hash>`).
2. **Android client** — Create credentials → OAuth client ID → *Android*, package `com.invoiceninja.admin`, and the SHA-1 of the **Play app-signing** key (Play Console → Setup → App integrity), plus one for the upload / debug key if you test local builds. Nothing from this client is passed to the app — it only has to exist in the same project as the Web client.
3. **Web client** — reuse the existing one (`640903115046-37ltu6s2j07gcqkssmf5feofj4isnsju.apps.googleusercontent.com`, admin-portal's `serverClientId`) or create a new *Web application* client. That ID is `IN_GOOGLE_SERVER_CLIENT_ID`.

**Wiring the values:**

- `ios/Runner/Info.plist` — add a second `CFBundleURLTypes` entry whose `CFBundleURLSchemes` holds the iOS URL scheme from step 1 (next to the existing `invoiceninja` one). The Google SDK throws before showing the chooser without it. It is public and safe to commit (admin-portal committed its own).
- CI — add the repo secrets `IN_GOOGLE_SERVER_CLIENT_ID` (read by `playstore.yml`) and `IN_GOOGLE_IOS_CLIENT_ID` (read by `appstore-ios.yml`).
- Local release builds — put both keys in `dev.json`; `tools/build_release.sh` resolves them env → `dev.json` → empty, like the Sentry DSN, and an Xcode **Product → Archive** picks up `IN_GOOGLE_IOS_CLIENT_ID` through the same `tools/xcode_inject_sentry_dsn.sh` pre-action.
- Debug runs — `flutter run --dart-define-from-file=dev.json` passes them too.

**The server side needs nothing.** The app rides Google's access-token path (`/api/v1/oauth_login` → `harvestUser`), which the server checks against Google's userinfo endpoint, not against a client ID. A brand-new Google account is created only through the sign-up screen, which sends `?create=true`; from the login screen the server answers "User not found".
