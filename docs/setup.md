...
**Xcode IDE archives (Product → Archive).** A hand-built archive from Xcode bakes the DSN in automatically — a `BuildAction` pre-action in both Runner schemes (`macos/`+`ios/Runner.xcodeproj/xcshareddata/xcschemes/Runner.xcscheme`) runs `tools/xcode_inject_sentry_dsn.sh`, which resolves the DSN with the **same** `env → dev.json → empty` order as `build_release.sh` and runs `flutter build <platform> --release --config-only --dart-define=IN_SENTRY_DSN=…` to write it into the generated xcconfig **before** the compile phase. It fires **only** for the Release configuration (Debug ⌘R / Profile runs are untouched); an empty DSN is a safe no-op logged as an Xcode `warning:`. Because GUI-launched Xcode (Finder/Dock) doesn't inherit shell env vars, **`dev.json` is the practical DSN source here** — `export IN_SENTRY_DSN=…` only reaches a terminal-launched Xcode. CLI/CI are unaffected: command-line `xcodebuild` and `flutter build` don't run scheme actions (that's why the pipelines below carry their own `--config-only` step). After a fresh clone, run one archive and confirm the DSN landed in the binary — `strings -a <archive>/…/App.framework/App | grep -c <your-sentry-host>` should be ≥ 1.
