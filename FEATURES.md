...
| List filter — checkbox multi-select for status / state | — | — | ✅ | ✅ | Lifecycle set is never empty (#126): clearing it returns to Active, so archived / deleted are opt-in; a key at its default renders no chip |
