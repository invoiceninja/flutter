...
class InvoiceRepository extends BaseEntityRepository<Invoice, InvoiceApi>
  ...
  Stream<List<Invoice>> watchRecent({
    ...
    if (limit <= 0) return Stream<List<Invoice>>.value(const <Invoice>[]);
    return db.invoiceDao
        .watchPage(
          companyId: companyId,
          offset: 0,
          ...
          sortAscending: false,
          tieBreakField: InvoiceFieldIds.createdAt,
          badgeModeId: badgeModeId,
        )
  ...
  ///
  /// A top-up, not a bundle: invoices still load page-by-page through
  ...
  Future<void> applyRefreshDelta({
    required String companyId,
    required List<InvoiceApi> bundle,
  }) => applyRefreshDeltaTemplate<InvoiceApi, InvoicesCompanion>(
    companyId: companyId,
    bundle: bundle,
    idOf: (a) => a.id,
    ...
    storedUpdatedAt: db.invoiceDao.updatedAtAmong,
  );
  ...
  Future<void> _writeDocuments(
    ...
    String json,
      ...
      (db.update(db.invoices)..where(
            (e) => e.companyId.equals(companyId) & e.id.equals(entityId),
          ))
          .write(InvoicesCompanion(documents: Value(json)));
}
