...
Companion to CLAUDE.md § Sync — non-obvious rules. The main file still states all 35 rules, one line each — it is the most-cited anchor in the repo. This doc carries the evidence behind the twenty-six that needed more than a line: the status-code map, the cursor and paging gates, the company-token guard, the offline-retry policy, and what a retry may safely repeat.
...
A ghost create is not deleted while another attempt at it is `in_flight` — a re-save queued while the first was on the wire: the discarded row goes alone, and the in-flight one, the local record and what was made against it stay for its landing or failure to settle (`sync_repository_test` › discard). Five more ways the tap, or the save after it, reached the wrong row. **A failed re-save drops the link to the dead row the form opened with.** Only the 422 arm did; after a 5xx the newer row sat `pending` while the VM still pointed at the old dead one, so Discard deleted that and the newer write — the one just discarded — applied anyway (`GenericEditViewModel.save`'s catch). The settings scaffold re-linked the newest dead row after *every* failed save, which undid that and swapped the fresh error for the old one; it now re-links only a server rejection with field errors, as `onSaveRejected` does. **A new record's rows are keyed on its temp id.** A create form's draft has no id, so every lookup keyed on `existingId` or the draft never ran: Discard after a 5xx left the create to go out, a 422's dead row was never linked, and the quick-create dialog's Cancel looked for dead rows only. Both scaffolds and the dialog fall back to `recoveryTempId`; the dialog reads it *before* the save, which clears it on success. A create form keeps its draft after discarding a create that never reached the server — it points at no record to leave — and the next Save mints a fresh temp id. A create that *may* have (`unconfirmed`) is different: discarding the one the banner shows leaves an `EntityEditScreenScaffold` create form, as a resent create does, so its draft can't be saved into a second record (a settings create form stays, as it always has — a known gap); and one that went `unconfirmed` out of sight — a background retry after the failure on screen — is shown, not dropped: the banner turns to Check / Resend / Discard and the form keeps its temp id, so a Save is refused (`UnconfirmedPriorMutationException`). The dialog's Cancel leaves such a row in the Outbox. **A landed create takes the dead attempts older than it.** `recordCreateSuccess`, while the drain dispatches that create, deletes the record's `dead` `create` rows older than it (`OutboxDao.deleteDeadCreates`) before `rewriteTempIdInPayloads` would re-key them into failed creates of a record that now exists — which a Retry would POST again. A newer dead attempt is kept: it holds newer content than what landed. **Only a create supersedes a failed create that has not landed.** An edit of a record whose create was rejected queues an *update*, which waits behind that create; the edit scaffold's `_cleanupPriorDeadRow` used to delete the create as superseded, and the update then died as referencing a discarded record. A dead create already under a real id is stale — the record exists — and is still superseded. **A stale banner doesn't discard what is on the wire.** The unconfirmed banner's Discard re-reads its row first, as Check and Resend do: resent from the Outbox meanwhile, a row that is only queued or has died since is still dropped, as asked, but an `in_flight` one — whose request discarding by id would leave on the wire — or a gone one only drops the banner. Pinned by `generic_edit_view_model_test` › a save that fails again, `settings_entity_edit_scaffold_save_failure_test` › Discard after a failed re-save and › a create form, `entity_edit_screen_scaffold_discard_test`, `client_create_dialog_test` › failed-attempt cleanup, `client_repository_test` › a create that finally succeeds, `outbox_dao_test` › deleteDeadCreates, and `save_failed_banner_resend_test` › Discard on a change resent elsewhere.
