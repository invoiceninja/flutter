...
- **A reset's outcome reaches the user once — a toast when everything came across, a dialog when unsynced work may not have — and a copy it keeps leaves the device only through Device Settings → Data.** → `docs/migrations.md` § What the user is told about a reset
