...
Six manually-triggered workflows publish the app — five to a store's testing channel, one (the Linux AppImage) to a GitHub Release — all `workflow_dispatch` only (a release is a deliberate act, never a per-push gate). **To run one:** GitHub → **Actions** → pick the workflow → **Run workflow** (on the default branch). Each builds the platform artifact, bakes in `IN_SENTRY_DSN`, and uploads to a non-production / testing channel (or a GitHub prerelease) — except Windows, which auto-commits to Store certification (see that section).

| Target | Workflow | Channel (non-prod) | Details |
|---|---|---|---|
| Linux Snap | `snapcraft.yml` | `edge` | § Linux desktop / Snap |
| Linux AppImage | `appimage.yml` | GitHub Release (prerelease) | § Linux desktop / AppImage |
...
**Finishing setup (one-time, per workflow).** The store accounts and listings already exist, but these workflows are new, so **their GitHub secrets are not configured yet** — before a workflow's first dispatch, add the secrets listed in its section (Settings → Secrets and variables → Actions) and do its one-time external setup. `IN_SENTRY_DSN` already exists and is reused by all of them. **The AppImage workflow is the exception** — it needs **no external secret** (only the built-in `GITHUB_TOKEN`), but as the first repo-writing workflow it requires the repo's Actions token policy to allow `contents: write` (Settings → Actions → General → Workflow permissions); GitHub Releases need no enabling, and a `v*` tag-protection rule (if ever added) would block it. The two Apple `ExportOptions.plist` files (+ the macOS `PROVISIONING_PROFILE_SPECIFIER`) are pre-filled with the expected profile names (`Invoice Ninja Admin App Store`, `Invoice Ninja macOS App Store`) — edit them only if the real portal profile names differ.

**Per release.** Run `tools/bump_client_version.sh`, commit, then dispatch. The build number must **strictly increase** for Play / iOS / macOS / Windows (each rejects a re-used version). **Snap is the exception** — it's revision-ordered, so no bump is needed to publish; its display version is maintained separately in `snap/snapcraft.yaml` (`version:`, currently `5.1.1`). **AppImage is the other exception** — its tag/asset come from the pubspec display version (`v<version>`), so re-dispatching the same version updates that GitHub Release in place; cut a *new* release by bumping the version (the bump script advances the patch, e.g. `5.1.4` → `5.1.5`, yielding a fresh `v5.1.5` tag).
...
- **NetworkManager under strict confinement (optional, NOT load-bearing).** `connectivity_plus` detects online/offline by querying NetworkManager over the system D-Bus (`package:nm`), which strict confinement blocks unless the `network-manager` plug is connected — also **not auto-connected**. Unlike the keyring plug this is *not* required: `ConnectivityWatcher` degrades a blocked/stalled probe to *assume-online* (`lib/data/services/connectivity_watcher.dart`), so saves and background sync work without it — only the offline banner + auto-drain-on-reconnect stay inactive until a user runs `snap connect invoiceninja:network-manager` (which resolves only where the host exposes the `network-manager` system slot — standard on NetworkManager distros). Because it's optional it is **not** a `stable`-release blocker. Skipping it was the cause of the old `org.freedesktop.DBus.Error.AccessDenied` "Could not save" on Linux, now fixed in the client regardless of the plug.
...
Alongside the Snap, the Linux app ships as an **AppImage** — a single self-contained executable that runs on any glibc-2.35+ distro with no install (issue #1). Built + published by the manually-triggered `.github/workflows/appimage.yml`, which attaches `InvoiceNinja-<version>-x86_64.AppImage` to a per-version GitHub **prerelease** (tag `v<version>` from pubspec). It reuses the snap's 512px icon (`snap/gui/invoiceninja.png`); its own desktop entry + AppStream metainfo live in `appimage/`.

- **Why also an AppImage.** The snap is effectively Ubuntu-centric and its strict confinement needs a manual `snap connect …:password-manager-service` or the encrypted DB won't open. An AppImage is **unconfined**, so `flutter_secure_storage` → libsecret → gnome-keyring works over D-Bus with **no manual step**. (The host must still run a Secret Service daemon — gnome-keyring/KWallet — for DB-key storage, the same underlying requirement as the snap.)
- **Build mechanism.** `tools/build_appimage.sh` (also the local builder — Linux host only) compiles `flutter build linux --release` with the DSN baked in, then packages via `linuxdeploy --plugin gtk` (bundles GTK3 + gdk-pixbuf loaders + GSettings schemas). The whole Flutter bundle goes into `AppDir/usr/bin/` so `admin`, `lib/`, and `data/` stay together — the embedder resolves `lib/libapp.so` + `data/` via `/proc/self/exe`. The tools (linuxdeploy, the gtk + appimage plugins) and the AppImage runtime are downloaded + checksum-verified (`appimage/tool-checksums.sha256`); CI sets `APPIMAGE_EXTRACT_AND_RUN=1` (the runners have no libfuse2). No `execstack` step (unconfined).
- **`ubuntu-22.04` pin.** Build on 22.04 (glibc 2.35) — an AppImage needs host glibc ≥ build glibc, so a newer build would refuse to start on older hosts. Same constraint/reason as the snap's core22.
- **Release cadence.** No external secret — only the built-in `GITHUB_TOKEN` (with `contents: write`). Re-dispatching the same version updates the release in place (replaces the asset); cut a new release by bumping the version (`tools/bump_client_version.sh` → fresh `v5.1.x` tag). Dry-run with the workflow's `publish: false` input (build + artifact, no release). x86_64 only for now (like the snap; arm64 is a future addition).
- **AppStream metadata.** `appimage/com.invoiceninja.admin.metainfo.xml` ships unvalidated on the first runs (`LDAI_NO_APPSTREAM=1`) because `appstreamcli validate` fails on warnings and the build can't be iterated locally; the workflow runs `appstreamcli validate` as a non-fatal diagnostic so the metainfo can be tightened and the flag dropped later. `<project_license>` is `AAL` (per `LICENSE.txt`) — confirm before flipping the release to stable.
...
**Xcode IDE archives (Product → Archive).** A hand-built archive from Xcode bakes the DSN in automatically — a `BuildAction` pre-action in both Runner schemes (`macos/`+`ios/Runner.xcodeproj/xcshareddata/xcschemes/Runner.xcscheme`) runs `tools/xcode_inject_sentry_dsn.sh`, which resolves the DSN with the **same** `env → dev.json → empty` order as `build_release.sh` and patches the `IN_SENTRY_DSN` entry straight into the generated xcconfig's base64 `DART_DEFINES` list **in place** — no nested `flutter build`. (Editing one line can't stall the archive the way a `flutter`/`pod` invocation inside a pre-action can — an earlier `flutter build … --config-only` version deadlocked iOS archives, which produced no `.xcarchive` — and it needs nothing on `PATH`.) Xcode reads `DART_DEFINES` as a build setting and `xcode_backend.sh`/`macos_assemble.sh` forward it to `flutter assemble`, so the compile picks up exactly what the pre-action wrote. It fires **only** for the Release configuration (Debug ⌘R / Profile runs are untouched); an empty DSN is a safe no-op logged as an Xcode `warning:`, as is a never-built tree with no `DART_DEFINES` yet (run any `flutter pub get`/build once first). Because GUI-launched Xcode (Finder/Dock) doesn't inherit shell env vars, **`dev.json` is the practical DSN source here** — `export IN_SENTRY_DSN=…` only reaches a terminal-launched Xcode. CLI/CI are unaffected: command-line `xcodebuild` and `flutter build` don't run scheme actions (that's why the pipelines below carry their own `--config-only` step). After a fresh clone, run one archive and confirm the DSN landed in the binary — `strings -a <archive>/…/App.framework/App | grep -c <your-sentry-host>` should be ≥ 1.

**CI publish pipelines.** The published artifacts are built by the per-target workflows (§ Shipping to the stores), each injecting the DSN from the `IN_SENTRY_DSN` GitHub Actions secret at compile time (`--dart-define=IN_SENTRY_DSN=${{ secrets.IN_SENTRY_DSN }}`). `tools/build_release.sh` is for **local** macOS/iOS/Android/web release builds; the Linux AppImage has its own `tools/build_appimage.sh`, which both compiles and packages (vs `build_release.sh linux`, which only compiles). (Note: there is no Dart-symbol upload pipeline — Sentry symbolication relies on symbols baked into the binary; see § Android release build before adding `--split-debug-info`.)
