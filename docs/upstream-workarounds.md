...
- Recheck trigger: when `RawAutocomplete` exposes a way to show its options view imperatively (or recomputes on focus) — drop the two hooks and confirm `test/ui/core/widgets/searchable_dropdown_field_test.dart` still passes, in particular "tapping again after a pick reopens the list". Note the public `showOptions()` / controller surface this waits on would also supply a first-class *hide*, which is the other half of the same missing API — see the `onTapOutside` bullet under **Considered but NOT tracked** below. They are separate problems (reopen vs. close) and the fixes are independent; don't collapse them.
...
## 12. printing: every native `sharePdf` writes the caller's file name as a raw path component, and swallows the failure

- Issue / waiting on: <https://github.com/DavBfr/dart_pdf> — no sanitisation on any platform and no error surfaced on three of four. macOS `PrintJob.swift:195-203` concatenates `NSTemporaryDirectory() + name` and `catch { print(…); return }`; iOS `PrintJob.swift:256-264` uses `appendingPathComponent(name)` with the same silent `return`; Android `PrintingJob.java:329,360` does `new File(shareDirectory, name)` inside `catch (IOException e) { e.printStackTrace(); }`; Windows `print_job.cpp:298-304` does `temp + "\\" + name` with an **unchecked** `ofstream` and then `ShellExecute`s a path that was never written. • Found: 2026-09-19 • printing: 5.14.3
- Symptom: Share / Download on a PDF does **nothing at all** — no share sheet, no file, no error, nothing in the log the user can see. Reachable with ordinary data: a saved-version label on the History tab is rendered through the company's `date_format_id`, which is `/`-separated on four of the fourteen formats **including the server's default** (`dd/MMM/yyyy`) and `:`-separated on all of them; and a document *number* pattern is a company setting, so `INV/2026/0001` is an ordinary European scheme. On Windows the colon alone breaks it for every company.
- Root cause: `/` and `\` are path separators, so the name becomes a path into a directory that does not exist; `:*?"<>|` are rejected outright by Win32. None of the four implementations validates, and three catch-and-discard the write error.
- Commit ref: not yet committed
- Change(s):
  - `lib/utils/file_names.dart` — **KEEP**: `sanitizeFileName`, which is correct regardless of what `printing` does. Also handles the Win32 reserved device names (`NUL.pdf` cannot be created even though every character is legal), a leading dot, a trailing dot or space, and a length cap.
  - the eleven call sites that interpolate a number or a date into a PDF file name (`lib/ui/features/billing_shared/pdf/billing_doc_pdf_view.dart`, `clients/views/client_statement_screen.dart`, and the download action + bulk-list action for each of invoice / quote / credit / purchase order / recurring invoice) — **KEEP**: route the name through it.
  - `test/lint/pdf_file_name_sanitized_test.dart` — **KEEP**: fails the build on a twelfth interpolated name that skips it.
- Accepted trade, recorded so it is not rediscovered: the mapping is **lossy**. Every illegal character becomes `-` and runs of `[-\s_]` collapse, so `INV/2026/1` and `INV-2026-1` — two numbers that can legitimately coexist in one company — both produce `invoice_INV-2026-1.pdf`, and two shared PDFs overwrite each other in the download directory. Strictly better than the current "nothing happens at all", and percent-encoding would produce names nobody can read.
- Revert: nothing to revert even if upstream fixes it — sanitising a file name before handing it to a platform is right either way. If `printing` ever starts *reporting* the failure, the value of the entry is that the silence was the reason this went unnoticed; delete the entry then and keep every change.
- Recheck trigger: a `printing` bump — check whether `sharePdf` sanitises or at least returns `false` on a write failure. If it reports errors, consider surfacing one instead of relying solely on sanitisation.
...
- `lib/utils/legacy_html_markdown.dart` — folds HTML into markdown before super_editor deserializes a note field. Not a package bug: the React client edits `public_notes` / `terms` / `footer` with TinyMCE and the pre-v5 apps used Quill, so HTML is a permanent inbound shape for fields this app stores as markdown. (`markdown`'s `HtmlBlockSyntax` parses a block-level tag as raw HTML running to the next blank line and super_editor's block visitor drops raw text — so `<ul>` used to delete the list *and* everything after it, invoiceninja/flutter#107; and `InlineHtmlSyntax` isn't in the default inline set, so `<strong>` renders verbatim.) The adapter stays even if super_editor ever grows HTML support.
- `lib/ui/core/widgets/markdown_text_field.dart` scoped `Theme(primaryColor: t.accent)` around the editor sliver — super_editor's **documented default** for the mobile caret, drag handles and magnifier is `Theme.of(context).primaryColor`, and `buildInTheme` leaves that to Flutter, which derives it as `colorScheme.surface` in dark mode: the caret was painted in the field's own background colour (invoiceninja/flutter#108). Not a package bug and nothing to revert — any host that leaves `ThemeData.primaryColor` underived in a dark theme hits it. (The obvious-looking `DefaultCaretOverlayBuilder` override beside it is desktop-only; `primaryColor` is the one seam that also reaches the Android handles and `SuperReader`.) Scoped rather than set globally in `buildInTheme` for a concrete reason: `printing` is the only other dependency that reads `ThemeData.primaryColor`, and `PdfPreview` uses it as its action-bar background — the app mounts three of those and sets `actionBarTheme` nowhere, so a global accent would have turned that bar blue in dark mode.
- `onTapOutside: dismissPickerOnTapOutside(focusNode)` on every `RawAutocomplete` field (`lib/ui/core/widgets/picker_dismissal.dart`; call sites in `searchable_dropdown_field.dart`, `client_picker_field.dart`, `tag_picker_field.dart`, and both cells in `line_item_editor/line_item_table_desktop.dart`) — on **native** Android/iOS touch the options popover had no dismissal path whatsoever. `_canShowOptionsView` is `hasFocus && options.isNotEmpty`, so only a pick, a `DismissIntent` (Escape — a hardware keyboard) or focus loss closes it; `_EditableTextTapOutsideAction` deliberately does **not** unfocus for a `PointerDeviceKind.touch` event on android/iOS/fuchsia unless `kIsWeb`; and a list with anything in it has no way out (`_optionsFor` never returns empty, and the tax cell prepends a "none" row; the create-bearing pickers can go empty, but not by any route a user can aim for). The list stayed up until the user picked something or left the screen — invoiceninja/flutter#130, reported against Activity's User filter but true of all ~120 picker call sites. **Not tracked** because `onTapOutside` is the *designed* seam for exactly this, the SDK behaviour it replaces is intentional and won't be "fixed" (a soft keyboard must not close on a stray tap), and the hooks stay correct either way — off native touch they reproduce the SDK default verbatim. Safe because `RawAutocomplete` wraps both the field and the options overlay in `TextFieldTapRegion` (same default `groupId: EditableText`), so a tap on an option row is inside the group. Pinned by `test/lint/picker_popover_wiring_test.dart`. Distinct from entry 7 above, which is about *reopening* the same overlay.
- `BackDismissiblePickerOverlay` in the same file, mounted inside every `RawAutocomplete`'s `optionsViewBuilder` — the back half of the bullet above, and **not tracked for the same reason plus a stronger one**: `RawAutocomplete` has no back seam at all to wait on (`autocomplete.dart` and `editable_text.dart` contain no `PopScope`, `BackButtonListener` or `NavigatorPopHandler`), and a `ChildBackButtonDispatcher` *is* the designed way for a non-route overlay to be consulted before `Router` pops. On Android, back with a popover open ran `SystemBackGate` → `NavHistoryController.back()` and left the screen — or `SystemNavigator.pop()` and exited the app, on a session restored straight onto that screen (invoiceninja/flutter#134). `BackDismissibleOverlay` in the same file is the general form and carries the sharp edge: a mounted wrapper out-ranks **every** modal route above it, so a menu row that pushes a date picker must dismiss *before* it awaits (`FilterSuggestionMenu.onDismiss`, `segment_menu.dart`). Its two non-picker hosts are `token_search_field.dart`'s suggestion and segment menus, pinned by `test/lint/overlay_back_dismiss_test.dart`. `back_dismissible_menu_anchor.dart` does the same job for `MenuAnchor` and is likewise absent from this ledger for the same reason — nothing to revert if Flutter changes, since a `ChildBackButtonDispatcher` is the designed seam. Note entry 7's hoped-for imperative `showOptions()` / controller API would supply the *hide* half of this too; if it lands, revisit both together — but the back **routing** would still need a hook, so this one shrinks rather than disappears.
